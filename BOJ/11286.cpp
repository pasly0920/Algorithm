...
int main()
{
    ...
    cout.tie(NULL);
    vector<int> heap_arr;
    vector<int> heap_arr_1;//음수
    heap_arr.push_back(0);
    heap_arr_1.push_back(0);
    int N;
    cin>>N;
    int X;
    for(int i=0;i<N;i++)
    {
        cin>>X;
        if(X==0)
            if((heap_arr.size()==1)&&(heap_arr_1.size()==1))
                cout<<0<<'\n';
            else if(heap_arr.size()==1)
                cout<<-1*heap_pop(heap_arr_1)<<'\n';
            else if(heap_arr_1.size()==1)
                cout<<heap_pop(heap_arr)<<'\n';
            else
                if(abs(heap_arr[1])>=abs(heap_arr_1[1]))
                    cout<<-1*heap_pop(heap_arr_1)<<'\n';
                else  
                    cout<<heap_pop(heap_arr)<<'\n';
        else 
